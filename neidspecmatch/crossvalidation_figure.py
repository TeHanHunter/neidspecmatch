...
COOL_LIMIT_K = 4500.0
EXPECTED_DRP_SERIES = package_utils.DEFAULT_LIBRARY_DRP_SERIES
...
def load_library_evidence(manifest_path: Path) -> dict[str, object]:
    ...
    strict_document = {
        ...
        "library_provenance": {
            ...
            for field in (
                "archive_products", "reference_dq_records",
                "source_archive", "source_catalog",
            )
        },
    }
    package_utils._validated_release_library_provenance(
        strict_document,
        expected_drp_series=EXPECTED_DRP_SERIES,
    )
